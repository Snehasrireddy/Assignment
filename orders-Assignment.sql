...
--WRITE A QUERY TO DISPLAY CUSTOMER FULL NAME WITH THEIR TITLE (MR/MS), BOTH FIRST NAME AND LAST NAME ARE IN UPPER CASE WITH 
...
select *,
     case
		 ...
		 Else ' '
		 end + customer_fname + ' ' + customer_lname as customer_fullname,
	 case 
         when customer_creation_date <'01-01-2005' Then 'A' 
         when customer_creation_date >='01-01-2005' and customer_creation_date <'01-01-2011' Then 'B'
         when customer_creation_date >='01-01-2011' Then 'C' 
...
select pc.product_class_code,pc.product_class_desc,count(p.product_class_code) as count_product_type,sum(cast(p.product_quantity_avail*p.product_price as int)) as inventory_value
 from product as p join product_class pc on p.product_class_code=pc.product_class_code 
 group by pc.product_class_code,pc.product_class_desc
 having sum(cast(p.product_quantity_avail*p.product_price as int)) > '100000' order by inventory_value desc;
...
-- [NOTE: TABLES TO BE USED - ONLINE_CUSTOMER, ADDRESSS, ORDER_HEADER]
 ...
 Select o.customer_id,CONCAT(o.customer_fname, ' ', o.customer_lname) AS customer_full_name,o.customer_email,o.customer_phone,a.country
from online_customer o
join address a on o.address_id=a.address_id
where o.customer_id IN(SELECT h.customer_id FROM order_header h group by o.customer_id
having count(*) = sum(case when h.order_status = 'Cancelled' then 1 else 0 end));
...
select s.shipper_name,a.city,count(distinct o.customer_id) as num_customers_catered,count(h.order_id) as num_consignments_delivered from shipper s
join order_header h on s.shipper_id = h.shipper_id 
join online_customer o on o.customer_id=h.customer_id
join address a on o.address_id=a.address_id
where s.shipper_name = 'DHL' group by s.shipper_name,a.city order by num_consignments_delivered  desc;
...
select o.customer_id,concat(o.customer_fname , ' ' , o.customer_lname) as customer_fullname,sum(cast(product_Quantity as int)) as Total_quantity,
sum(cast(product_quantity * product_price as int)) as Total_value from online_customer o  
join order_header h on o.customer_id=h.customer_id 
join order_items i on h.order_id=i.order_id
join product p on i.product_id=p.product_id 
where payment_mode = 'cash' and customer_lname like 'G%'
group by o.customer_id,concat(customer_fname , ' ' , customer_lname)
order by Total_value desc;

-- 7. WRITE A QUERY TO DISPLAY ORDER_ID AND VOLUME OF BIGGEST ORDER (IN TERMS OF VOLUME) THAT CAN FIT IN CARTON ID 10  
select Top 1  i.order_id,sum(cast(p.len * p.width * p.height * i.product_quantity as int)) as volume from order_items i
join product p on i.product_id=p.product_id
group by i.order_id 
having sum(cast(p.len * p.width * p.height * i.product_quantity as int)) <= 
(select (c.len*c.width*c.height) as volume from carton c 
where carton_id = 10) 
...
select p.product_id,p.product_desc,p.product_quantity_avail,coalesce(sum(i.product_quantity),0) as quantity_sold,
case
    when sum(cast(i.product_quantity as int)) = 0 Then 'NO SALES IN PAST, GIVE DISCOUNT TO REDUCE INVENTORY'
	when p.product_quantity_Avail < 0.5 * sum(cast(i.product_quantity as int)) Then 'LOW INVENTORY, NEED TO ADD INVENTORY'
        else 'NORMAL STATUS'
	end as inventory_status
from product p 
left join order_items i on p.product_id = i.product_id 
join product_class c on p.product_class_code=c.product_class_code 
where c.product_class_desc IN ('Electronics','Computer')
...
case
    when sum(cast(i.product_quantity as int)) = 0 Then 'NO SALES IN PAST, GIVE DISCOUNT TO REDUCE INVENTORY'
	when p.product_quantity_Avail < 0.2 * sum(cast(i.product_quantity as int)) Then 'LOW INVENTORY, NEED TO ADD INVENTORY'
	when p.product_quantity_Avail < 0.6 * sum(cast(i.product_quantity as int)) Then  'MEDIUM INVENTORY, NEED TO ADD SOME INVENTORY'
        else  'SUFFICIENT INVENTORY'
	end as inventory_status
from product p 
left join order_items i on p.product_id = i.product_id 
join product_class c on p.product_class_code=c.product_class_code 
where c.product_class_desc IN ('Mobiles','Watches')
group by p.product_id,p.product_desc,p.product_quantity_avail
order by p.product_id;
...
case
    when sum(cast(i.product_quantity as int)) = 0 Then 'NO SALES IN PAST, GIVE DISCOUNT TO REDUCE INVENTORY'
	when p.product_quantity_Avail < 0.3 * sum(cast(i.product_quantity as int)) Then 'LOW INVENTORY, NEED TO ADD INVENTORY'
	when p.product_quantity_Avail < 0.7 * sum(cast(i.product_quantity as int)) Then  'MEDIUM INVENTORY, NEED TO ADD SOME INVENTORY'
        else  'SUFFICIENT INVENTORY'
	end as inventory_status
from product p 
left join order_items i on p.product_id = i.product_id 
join product_class c on p.product_class_code=c.product_class_code 
where c.product_class_desc IN ('Mobiles','Watches','Electronics','Computer')
group by p.product_id,p.product_desc,p.product_quantity_avail
order by p.product_id;
...
select p.product_id,product_desc,sum(cast(product_Quantity as int)) as Total_quantity from product p
join order_items i on p.product_id=i.product_id 
join order_header h on i.order_id=h.order_id 
join online_customer o on h.customer_id=o.customer_id
join address a on o.address_id=a.address_id
where h.order_id  IN(select i.order_id from order_items where i.product_id = '201' ) and i.product_id != '201' and a.city not in('Banglore','New_Delhi')
...
select h.order_id,o.customer_id,concat(customer_fname,' ',customer_lname) as customer_fullname,sum(cast(i.product_Quantity as int)) as Total_quantity 
from online_customer o
join order_header h on o.customer_id=h.customer_id
join order_items i on h.order_id=i.order_id
join address a on o.address_id=a.address_id 
where h.ORDER_ID % 2 = 0 and a.pincode not like '%5' 
group by h.order_id,o.customer_id,concat(customer_fname,' ',customer_lname)
order by h.order_id;
